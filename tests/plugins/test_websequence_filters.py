...
@patch("dexy.filters.websequence.urlopen", side_effect = [StringIO("?png=whereToGo"), StringIO("mock_result")])
def test_websequence_svg(mocked_urlopen):
    ...
    assert mocked_urlopen.call_args_list[0] == call("http://www.websequencediagrams.com/index.php",
    	'message=Sender-%3EReceiver&style=default&apiVersion=1')
    assert mocked_urlopen.call_args_list[1] == call("http://www.websequencediagrams.com/?png=whereToGo")